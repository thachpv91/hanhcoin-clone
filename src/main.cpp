...
uint256 hashGenesisBlock("0x32faf033cd528f7c245f864b5f64d7b1f84b93be8105a110311b03365870d7fb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hanhcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hanhcoin Signed Message:\n";
...
{
    // Hanhcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Hanhcoin: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hanhcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hanhcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Hanhcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x32faf033cd528f7c245f864b5f64d7b1f84b93be8105a110311b03365870d7fb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Birthday of HanhCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x4ff1fd91a9db99118ff72ecacc6d370c56b26ca1acfda769bd2db989ca8b2d25"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Hanhcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HanhcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HanhcoinMiner : ProcessBlock, block not accepted");
    }
